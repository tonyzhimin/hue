...
def listdir(request, path):
    ...
    data = {
        ...
        'current_request_path': reverse('filebrowser.views.view', kwargs=dict(path=urllib.quote(path.encode('utf-8'), safe=SAFE_CHARACTERS_URI_COMPONENTS))),
...
def _massage_stats(request, stats):
    ...
    return {
        ...
        'url': reverse('filebrowser.views.view', kwargs=dict(path=urllib.quote(normalized.encode('utf-8'), safe=SAFE_CHARACTERS_URI_COMPONENTS))),
        'is_sentry_managed': request.fs.is_sentry_managed(path)
    }
